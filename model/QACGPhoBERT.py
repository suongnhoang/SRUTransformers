...
global init_weights
init_weights = WeightsInitializer()
...
class RobertaOutput(nn.Module):
    def __init__(self, config, sru=False):
        super().__init__()
        self.sru = sru
        if not sru:
...
class RobertaIntermediate(nn.Module):
    def __init__(self, config, sru):
        super().__init__()
        self.sru=sru
        if not sru:
            self.dense = nn.Linear(config.hidden_size, config.intermediate_size)
        else:
            self.sru = SRU(config.hidden_size, config.hidden_size, num_layers=1, dropout=0.2, bidirectional = False, layer_norm = False, highway_bias = -2,)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.sru:
            hidden_states = self.sru(hidden_states)[0]
        else:
            hidden_states = self.dense(hidden_states)
...
class RobertaLayer(nn.Module):
    def __init__(self, config, sru):
        ...
        self.intermediate = RobertaIntermediate(config, sru)
        self.output = RobertaOutput(config, sru)
...
class RobertaEncoder(nn.Module):
    def __init__(self, config, sru=False):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayer(config, sru) for _ in range(config.num_hidden_layers)])
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, model_path=None, use_sru=False):
        super().__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = RobertaEncoder(config, use_sru)
    ...
    def load_trained_weights(self):
        ...
        incompatibleKeys = self.load_state_dict(checkpoint, strict=False)
        ...
        if len(unexpected_keys):
            ...
            log_string += f"try to init new params for {unexpected_keys}"
            print(log_string)
        if len(missing_keys):
            log_string = f"We found {len(missing_keys)} {f'missing_keys='.split('=')[0]}, "
            log_string += f"try to init new params for {missing_keys}"
            print(log_string)
